...
<div class="row">
  <div class="col-md-12">
    <div class="panel">
      <div class="panel-heading clearfix" style="background: linear-gradient(to right, #80FFFF, #70E0FF, #64D6FF, #5DC8FF, #43BFFF); color:black">
        <strong>
          ...
          <tbody id="product_info">
            <tr>
              <form method="post" action="edit_sale.php?id=<?php echo (int)$sale['id']; ?>">
                <td id="s_item_id">
                  <input type="text" class="form-control" id="s_item_id_input" name="item_id" value="<?php echo $product['itemID']; ?>" readonly>
                </td>
                <td id="s_name">
                ...
                <input type="hidden" name="s_id" value="<?php echo $sale['product_id']; ?>">
              </form>
            </tr>
          </tbody>
        </table>
...
$(document).ready(function() {
  $('#sug_input').on('input', function() {
    var partial_name = $(this).val(); // Get the partial input text
    if (partial_name.length > 0) {
      $.ajax({
        url: 'ajax.php',
        type: 'POST',
        data: { product_name: partial_name }, // Send partial input text to the server
        dataType: 'json',
  ...
  $(document).on('click', '.list-group-item', function() {
    ...
    $('#s_item_id_input').val(itemID);
    $('#sug_input').val(name);
    $('input[name="s_id"]').val(id);
